...
def read_texts_with_labels(csv_path: str, text_col: str = "text", label_col: str = "topic", limit: int = Limit) -> List[List[str]]:
    docs, labels, i = [], [], 0
...
def objective(trial: optuna.Trial, docs_tok: List[List[str]], docs_labs: List[List[str]], seed: int) -> float:
    ...
    model = tp.PLDAModel(alpha=alpha, eta=eta, seed=seed,
                        min_cf=min_cf, rm_top=rm_top)

    for doc, lab in zip(docs_tok, docs_labs):
        model.add_doc(doc, labels=lab)
...
def main():
    ...
    ap.add_argument("--out", default="runs_llda")
    ...
    docs_tok, docs_labs = read_texts_with_labels(args.train)
    ...
    study.optimize(lambda tr: objective(tr, docs_tok, docs_labs, args.seed),
    ...
    for rank, tr in enumerate(best_trials, start=1):
        ...
        model = tp.PLDAModel(alpha=p["alpha"], eta=p["eta"],
        ...
        # сохраняем модель
        model_path = os.path.join(args.out, f"{run_start}_model_rank{rank}_ppl{ppl:.2f}_div{div:.2f}.bin")
        model.save(model_path, full=True)
        logging.info(f"Saved to {model_path}. Generating pyLDAvis report...")

        # PyLDAVis (ручная сборка)
        vis_path = os.path.join(args.out, f"{run_start}_pyldavis_rank{rank}.html")
    ...
    for r in results:
        str_res = f"[rank {r['rank']}] ppl={r['perplexity']:.2f} div={r['diversity']:.2f}" + \
