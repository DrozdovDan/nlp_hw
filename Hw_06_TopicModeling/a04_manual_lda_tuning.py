...
def main():
    ...
    ap.add_argument("--out", default="runs_lda_custom")
    ...
    ap.add_argument("--min_cf", type=int, default=20)
    ap.add_argument("--rm_top", type=int, default=40)
    ...
    # сохраняем модель
    model_path = os.path.join(args.out, f"{run_start}_model_k{p['k']}_ppl{ppl:.2f}_div{div:.2f}.bin")
    model.save(model_path, full=True)
    logging.info(f"Saved to {model_path}. Generating pyLDAvis report...")

    # PyLDAVis (ручная сборка)
    vis_path = os.path.join(args.out, f"{run_start}_pyldavis_k{p['k']}.html")
    ...
    with open(os.path.join(args.out, f"{run_start}_summary.json"), "w", encoding="utf-8") as f:
    ...
    for r in results:
        str_res = f"ppl={r['perplexity']:.2f} div={r['diversity']:.2f}  k={r['params']['k']}" + \
